...
class ReportController extends Controller
{
    ...
    /**
     * Customer wise payment sheet for the accountant: only the customer column is
     * filled in, payment date / account / transaction id stay blank so they can be
     * written by hand on the printed copy.
     */
    public function customerPaymentSheetReport(Request $request)
    {
        try {
            $monthYear = $request->input('month_year');
            ...
            // Every customer who has an order in the selected month
            $customerIds = Order::where('user_id', auth()->id())
                ->whereRaw("DATE_FORMAT(COALESCE(order_date, DATE(created_at)), '%Y-%m') = ?", [$monthYear])
                ->distinct()
                ->pluck('customer_id');
            ...
            $rows = Customer::whereIn('id', $customerIds)
                ->get()
            ...
            try {
                ...
                $pdf = \PDF::loadView('admin.monthly-report.customer-payment-sheet', $data);
                $pdf->setPaper('A4', 'portrait');
            ...
            Log::error('ReportController@customerPaymentSheetReport Error: ' . $th->getMessage());
            return redirect()->back()->with('error', $th->getMessage());
        }
    }
}
